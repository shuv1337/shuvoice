...
def test_downsample_integer_ratio_uses_average_filter():
    audio = AudioCapture()

    source = np.array([0, 3, 6, 9, 12, 15], dtype=np.float32)
    out = audio._downsample_integer_ratio(source, 3)

    assert np.allclose(out, np.array([3.0, 12.0], dtype=np.float32))
...
def test_downsample_integer_ratio_keeps_carry_between_calls():
    ...
    assert np.allclose(out2, np.array([7.0 / 3.0, 20.0 / 3.0], dtype=np.float32))
